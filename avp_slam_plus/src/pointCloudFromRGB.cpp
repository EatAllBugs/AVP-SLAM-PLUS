...
void transformToFrame(PointType const* const pi,
                      PointType* const po,
                      Eigen::Quaterniond q,
                      Eigen::Vector3d t) {
...
void removeUnsynData(double& timeMax,
                     double& timeCameraImage,
...
void calCloudFromImage(Eigen::Matrix3d& K,
                       Eigen::Matrix3d& RT,
                       const cv::Mat& image,
    ...
    for (int i = 0; i < row; i = i + imageRowIncrease) {
        ...
        for (int j = 0; j < col; j = j + imageColIncrease) {
            ...
            int g = p[3 * j + 1];
            int r = p[3 * j + 2];
            //   there,for simplicity,just according to color,detect invalid
            //   area like sky area; for real scene,should adapt machine
            //   learning method or other method detecting invalid area
            if (b == skyColor) {
                break;
            }
    ...
    while (ros::ok()) {
        ...
        if (!camera0ImageBuf.empty() && !camera1ImageBuf.empty() &&
            ...
            if ((timeCamera0Image != timeCamera1Image) ||
                ...
                removeUnsynData(timeImageMax, timeCamera0Image,
                                camera0ImageBuf);
                removeUnsynData(timeImageMax, timeCamera1Image,
                                camera1ImageBuf);
                removeUnsynData(timeImageMax, timeCamera2Image,
                                camera2ImageBuf);
                removeUnsynData(timeImageMax, timeCamera3Image,
                                camera3ImageBuf);
                removeUnsynData(timeImageMax, timeCamera4Image,
                                camera4ImageBuf);
                removeUnsynData(timeImageMax, timeCamera5Image,
                                camera5ImageBuf);
